              ...
              <v-col>
                <div>
                  ...
                  <div v-if="$vuetify.display.mdAndUp">
                    <p></p>
                    <iframe :src="embeddedUrl(venue)" width="400" height="300" frameborder="0"
                      style="border: 0"></iframe>
                  </div>
                  <div v-if="$vuetify.display.smAndDown"><a :href="linkUrl(venue)" target="_blank">map</a></div>
                </div>
                <div>
...
function makeParts(venue: Venue) {
  return ["https://maps.google.com/maps?&q=", encodeURIComponent(`${venue.name} ${venue.address}`.replace(/\s/g, "+")), "&output=embed"]
}
