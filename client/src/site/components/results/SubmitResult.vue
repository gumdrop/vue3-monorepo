<template>
  ...
  <v-col align-center style="padding-left:48%;"><v-progress-circular v-if="!fixture" indeterminate
  ...
  <v-form v-model="valid" v-if="fixture">
    <v-col>
      ...
      <div v-if="!fixture.result">
        ...
        <v-text-field v-model.number="result.awayScore" :rules="[required('Away Score')]" :label="fixture.away.name"
          type="number"></v-text-field>
      </div>

      <v-textarea v-model="reportText" outline auto-grow label="Match Report">
        <template v-slot:append><v-tooltip top><template v-slot:activator="{ props }"><v-btn v-on="props" small flat
                @click="preview = !preview"><v-icon
      ...
      <transition name="fade">
        <v-card v-if="preview">
          <v-card-title><v-icon color="light-blue">mdi-eye-outline</v-icon>
...
import { Fixture, Result } from '@/entity/Fixtures';
