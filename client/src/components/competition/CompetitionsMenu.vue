<template>
  <div>
    <QlSideMenu :title="'Competitions ' + formatSeason(season)" icon="mdi-trophy" v-if="competitions && season">
      <SideMenuItem v-for="competition in competitions" :key="competition.id"
        :to="`/competition/${encode(competition.path)}/${competition._name}`" :prepend-icon="competition.icon"
