<template>
  <v-container v-if="fixtures" :class="gridSize" fluid>
    <v-col v-for="fixs in fixtures" :key="fixs.id">
      <FixturesCard :fixtures="fixs" />
    </v-col>
  </v-container>
</template>
...
const fixtures = usePromise(() => spentFixtures(`${seasonId?.value}`))
