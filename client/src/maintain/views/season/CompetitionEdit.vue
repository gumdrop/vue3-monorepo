<template>
  <v-container v-if="competition">
    <v-form ref="form" v-model="valid">
      <v-card>
        <v-card-title>
          {{ isNew ? 'Add' : 'Edit' }} Competition
        </v-card-title>
        <v-card-text>
          <v-text-field v-model="competition.name" label="Name" :rules="[rules.required('Name')]"></v-text-field>
          <v-select v-model="competition._name" :items="['league', 'cup', 'subsidiary', 'singleton']" label="Type"
            :rules="[rules.required('Type')]" :disabled="!isNew"></v-select>
          <v-text-field v-model.number="competition.duration" label="Duration" type="number"></v-text-field>

          <template
            v-if="competition._name === 'league' || competition._name === 'cup' || competition._name === 'singleton'">
            <v-text-field v-model="competition.startTime" label="Start Time" placeholder="20:00"></v-text-field>
          </template>

          <template v-if="competition._name === 'league'">
            <v-row>
              <v-col><v-text-field v-model.number="competition.win" label="Win Points"
                  type="number"></v-text-field></v-col>
              <v-col><v-text-field v-model.number="competition.draw" label="Draw Points"
                  type="number"></v-text-field></v-col>
              <v-col><v-text-field v-model.number="competition.loss" label="Loss Points"
                  type="number"></v-text-field></v-col>
            </v-row>
          </template>
    ...
    <v-card class="mt-4" v-if="!isNew && (competition._name === 'league' || leagueTables.length > 0)">
      <v-card-title>
        League Tables
        <v-spacer></v-spacer>
      ...
      <v-list>
        <template v-if="leagueTables.length">
          <v-list-item v-for="table in leagueTables" :key="table.id" @click="editLeagueTable(table)">
...
const editLeagueTable = (table: LeagueTable) => {
  router.push(`/season/${seasonId.value}/competition/${competition.value.id}/leaguetable/${table.id}`)
}
</script>
